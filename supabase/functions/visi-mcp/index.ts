...
const mcp = new McpServer({ name: "visios-mcp", version: "1.0.0" });
...
mcp.tool({
  ...
  handler: async () => {
    ...
    const [orgsRes, projRes, taskRes, notifRes] = await Promise.all([
      admin.from("orgs").select("id, name, slug").in("id", orgIds.length ? orgIds : ["00000000-0000-0000-0000-000000000000"]),
      admin.from("projects").select("id, name, emoji, org_id, status").eq("is_archived", false).in("org_id", orgIds.length ? orgIds : ["00000000-0000-0000-0000-000000000000"]),
      admin.from("tasks").select("id, title, status, priority, due_at, org_id, project_id, assignee_id").neq("status", "done").in("org_id", orgIds.length ? orgIds : ["00000000-0000-0000-0000-000000000000"]).order("due_at", { ascending: true, nullsFirst: false }).limit(100),
      admin.from("notifications").select("id, title, body, severity, org_id, created_at").is("acknowledged_at", null).in("org_id", orgIds.length ? orgIds : ["00000000-0000-0000-0000-000000000000"]).order("created_at", { ascending: false }).limit(50),
    ]);
    return text({
      orgs: orgsRes.data ?? [],
...
mcp.tool({
  ...
  handler: async (args: any) => {
    const orgIds = await userOrgIds();
    let q = admin.from("tasks").select("*").in("org_id", orgIds.length ? orgIds : ["00000000-0000-0000-0000-000000000000"]);
...
mcp.tool({
  ...
  handler: async (args: any) => {
    const orgIds = await userOrgIds();
    let q = admin.from("projects").select("*").eq("is_archived", false).in("org_id", orgIds.length ? orgIds : ["00000000-0000-0000-0000-000000000000"]);
    if (args?.org_id) q = q.eq("org_id", args.org_id);
...
mcp.tool({
  ...
  handler: async (args: any) => {
    const orgIds = await userOrgIds();
    let q = admin.from("notifications").select("*").is("acknowledged_at", null).in("org_id", orgIds.length ? orgIds : ["00000000-0000-0000-0000-000000000000"]);
...
mcp.tool({
  ...
  description: "Update a task's status, priority, due date, title, or assignee.",
  inputSchema: {
    type: "object",
    properties: {
      task_id: { type: "string" },
  ...
  handler: async (args: any) => {
    ...
    for (const k of ["title", "status", "priority", "due_at", "assignee_id", "description"]) {
...
mcp.tool({
  ...
  inputSchema: {
    ...
    properties: {
      ...
      body: { type: "string" },
      severity: { type: "string", enum: ["critical", "warn", "info"] },
      app: { type: "string", description: "Source app/integration label, e.g. 'mcp', 'claude'" },
...
mcp.tool({
  ...
  inputSchema: {
    type: "object",
    properties: {
      tool: { type: "string", description: "Name of the tool/action being proposed" },
      payload: { type: "object", description: "Proposed arguments / action details" },
      org_id: { type: "string" },
      note: { type: "string" },
    },
...
mcp.tool({
  ...
  inputSchema: {
    ...
    properties: {
      task_id: { type: "string" },
      body: { type: "string" },
      kind: { type: "string", description: "default 'comment'" },
      metadata: { type: "object" },
    },
