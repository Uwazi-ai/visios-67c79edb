/**
 * Kova MCP Server — team-wide, Streamable HTTP (stateless JSON-RPC)
 *
 * Auth:
...
async function resolveUserId(admin: SupabaseClient, token: string): Promise<string | null> {
  ...
  // Shared bearer secret (maps to VISI_MCP_USER_ID)
  const sharedSecret = Deno.env.get("VISI_MCP_SECRET");
  if (sharedSecret && envUser && token === sharedSecret) return envUser;
  // Legacy env-var token fallback
...
const TOOLS = [
  { name: "visi_get_context", description: "Get the full Kova context: your orgs, projects, open task counts, recent notifications. Call this first.", inputSchema: { type: "object", properties: {}, required: [] } },
  ...
  { name: "visi_notify", description: "Push a notification into Kova (FYI-style, no approval needed).", inputSchema: { type: "object", properties: { app: { type: "string" }, title: { type: "string" }, body: { type: "string" }, severity: { type: "string", enum: ["info", "warning", "critical"] }, metadata: { type: "object" }, org_id: { type: "string" } }, required: ["app", "title"] } },
  { name: "visi_request_approval", description: "Queue an action for human approval inside Kova.", inputSchema: { type: "object", properties: { title: { type: "string" }, body: { type: "string" }, source: { type: "string" }, org_id: { type: "string" }, priority: { type: "string", enum: ["urgent", "high", "medium", "low"] }, metadata: { type: "object" } }, required: ["title", "source"] } },
  ...
  { name: "visi_search_kb", description: "Full-text search across the Kova knowledge base.", inputSchema: { type: "object", properties: { query: { type: "string" }, org_id: { type: "string" }, limit: { type: "number" } }, required: ["query"] } },
  ...
  { name: "visi_send_chat_message", description: "Post a message to a named Kova channel.", inputSchema: { type: "object", properties: { channel: { type: "string", description: "Channel name (e.g. 'general', 'dailyreports')" }, message: { type: "string", description: "Message content" }, org_id: { type: "string", description: "Optional org_id to scope channel lookup" } }, required: ["channel", "message"] } },
  ...
  switch (method) {
    case "initialize": {
      ...
      return mcpOk(id, {
        ...
        serverInfo: { name: "kova-mcp", version: "2.0.0" },
        instructions: "Kova MCP — call visi_get_context first to discover orgs, projects, open tasks, and recent notifications. All tool calls are scoped to your user and org memberships.",
      });
    }
